...
import timm
from models import CTMFNet

from Attention.AOAsM import AoAsm
from Attention.MFC import AttentionFramework
from Attention import CBAM, SGCA, LaplacianAttention, GLCMAttention_GPU
...
if __name__ == "__main__":
    ...
    SAVE_PATH = '../result/'
    filename = '{}our-pneumonia_model'.format(SAVE_PATH)  # 文件扩展名在保存时添加
    ...
    image_datasets = {
        x: DataClass(split=x,
                     transform=data_transforms[x], size=224, download=True) for x in ['train', 'test']}

    dataloaders: dict = {
        x: data.DataLoader(
            image_datasets[x], batch_size=batch_size, drop_last=True, shuffle=True if x == 'train' else False
        ) for x in ['train', 'test']
    }

    model_ft = CTMFNet.cnnvit_any(num_classes=CLASSES)
    print(model_ft)
    total_params_o = sum(p.numel() for p in model_ft.parameters())
    print("添加前模型的参数量：", total_params_o)
    # 添加模块
    class Causal(nn.Module):
        def __init__(self, channels, batchsize):
            super(Causal, self).__init__()
            self.aoasm = AoAsm(input_channels=channels, latent_dim=16, batchsize=batchsize)

        def forward(self,original_feature, attention_feature):
            x = self.aoasm(original_feature, attention_feature)
            return x

    class Att_framework(nn.Module):
        def __init__(self, input_channels):
            super(Att_framework, self).__init__()

            attention_modules = [
            CBAM.CBAMBlock(input_channels, reduction=8, kernel_size=1),
            SGCA.SpatiatGroupCrosschannelAttention(groups=8),
            LaplacianAttention.LaplacianAttention(input_channels),
            GLCMAttention_GPU.GLCMAttention(input_channels)
        ]
            self.framework = AttentionFramework(attention_modules, input_channels)

        def forward(self,x):
            x = self.framework(x)
            return x

    class Mutil_Causal(nn.Module):
        def __init__(self, input_ch, batch):
            super(Mutil_Causal, self).__init__()
            self.mutilatt = Att_framework(input_channels=input_ch)
            self.causal = Causal(channels=input_ch, batchsize=batch)

        def forward(self, x):
            matt_out = self.mutilatt(x)
            causal_out = self.causal(x, matt_out)
            m_c_out = 0.7 * x + 0.25 * matt_out + 0.05 * causal_out
            return m_c_out
    # 模型修改
    model_ft.network[0].add_module('A1layer', Mutil_Causal(input_ch=56, batch=batch_size))
    model_ft.network[2].add_module('A1layer', Mutil_Causal(input_ch=112, batch=batch_size))
    model_ft.network[4].add_module('A1layer', Mutil_Causal(input_ch=224, batch=batch_size))
    model_ft.network[6].add_module('A1layer', Mutil_Causal(input_ch=448, batch=batch_size))

    print(model_ft)
    # 计算的参数量
    total_params_o = sum(p.numel() for p in model_ft.parameters())
    print("添加后模型的参数量：", total_params_o)
    ...
    accdf.to_csv('my-pneumonia-acc.csv')
    lossdf = pd.DataFrame(lossnp)
    lossdf.to_csv('my-pneumoniamnist-loss.csv')
    # 训练结束
    time_elapsed = time.time() - since
    print()
    print('任务完成！')
